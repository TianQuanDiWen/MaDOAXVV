...
默认预设按玩家常规日常动线串联，支持**无人值守全流程一键托管**：

```mermaid
flowchart TD
    Start([点击开始]) --> Launch[1. 启动游戏 & 启动器点击]
    Launch --> Login[2. 登录游戏 / 跳过动画 / 签到 / 关公告]
    Login --> MainLoop[3. 依次执行日常任务链]
    
    subgraph DailyTasks [日常任务支持]
        direction TB
        T1[🎁 抽免费扭蛋] --> T2[♨️ 岛主房间温泉与工作]
        T2 --> T3[⚔️ 自动清理新比赛]
        T3 --> T4[🎫 每日自动挑战券]
        T4 --> T5[🏆 每日活动挑战 & 排位赛]
        T5 --> T6[📬 一键领邮箱与每日任务奖励]
    ...
    MainLoop --> DailyTasks
    DailyTasks --> Exit[4. 正常菜单退出游戏 (可选)]
    Exit --> Done([完成日常，释放系统资源])
```
